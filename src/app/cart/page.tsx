...
export default function CartPage() {
    ...
    return (
        <main className="min-h-screen bg-white dark:bg-gray-950 pb-32">
                    ...
                    <div key={item.product.id} className="flex gap-4 p-3 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800">
                        ...
                        <div className="flex-1 flex flex-col justify-between">
                            <div>
                                ...
                                <p className="text-xs text-gray-500 dark:text-gray-400">{item.quantity} x {new Intl.NumberFormat('en-ET', { style: 'currency', currency: 'ETB' }).format(item.product.price)}</p>
                            </div>
                            <div className="flex items-center justify-between mt-2">
                                <span className="font-bold text-gray-900 dark:text-white">
                                    {new Intl.NumberFormat('en-ET', { style: 'currency', currency: 'ETB' }).format(item.product.price * item.quantity)}
                                </span>
                                <button
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-white dark:bg-gray-950 border-t border-gray-100 dark:border-gray-800 max-w-md mx-auto">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-2xl font-bold text-gray-900 dark:text-white">
                        {new Intl.NumberFormat('en-ET', { style: 'currency', currency: 'ETB' }).format(totalPrice)}
                    </span>
                </div>
                <button
                    onClick={handleCheckout}
                    className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 px-4 rounded-xl shadow-lg shadow-blue-500/30 active:scale-[0.98] transition-all"
                >
                    Checkout
                </button>
            </div>
